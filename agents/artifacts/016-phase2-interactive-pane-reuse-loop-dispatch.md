# Dispatch: Phase 2 Interactive Pane Reuse Loop
...
## Why
The previous Phase 2 smoke proved the happy path `Main → Plan → Dev → Review → Main`. This dispatch validates the next critical behavior: when Review routes back to Dev in the same dispatch, dispatch-auto should reuse the already-running Dev interactive Pi pane/session for the second Dev turn instead of launching a new Dev pane.
...
1. First Dev pass writes `agents/artifacts/016-phase2-interactive-pane-reuse-loop-marker.md` with exact line:
...
3. Second Dev pass updates the marker to the second-pass line and updates/creates `agents/artifacts/016-phase2-interactive-pane-reuse-loop-complete.md` with lightweight verification results.
...
## Acceptance Criteria
- [ ] Plan writes `agents/artifacts/016-phase2-interactive-pane-reuse-loop-plan.md` with a tiny artifact-only implementation plan that explicitly requires the forced Review → Dev loop.
- [ ] First Dev writes marker with exact line `phase2-reuse-loop-state: first-pass`.
- [ ] First Review writes/updates `agents/artifacts/016-phase2-interactive-pane-reuse-loop-review.md` and returns `State = needs-dev-fix` to Dev.
- [ ] Second Dev updates marker to exact line `phase2-reuse-loop-state: second-pass` and writes/updates `agents/artifacts/016-phase2-interactive-pane-reuse-loop-complete.md`.
