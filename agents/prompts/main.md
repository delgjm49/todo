...
Use `main` as the first line of a new session to activate the Main Orchestrator. Main may also be resumed through a dispatch spool directory with `pickup agents/channels/###-feature/` when the latest message file is addressed to Main.
...
Auto-orchestration note: if `agents/orchestration.json` exists in this repo, the `dispatch-auto` Pi extension will automatically spawn Plan / Dev / Review workers as soon as your turn ends with a fresh spool message file addressed from Main to Plan or Dev. **Phase 3 channel format is mandatory for new dispatches:** create `agents/channels/<slug>/messages/001-main-to-plan.md`; do **not** create legacy `agents/channels/*-channel.md` files. Old smoke channels are historical examples only, not templates. The chain runs autonomously until it routes back to Main. In this repo, dispatch-system tests/smoke tests should assume **interactive** worker mode by default unless the user explicitly asks for subprocess mode; set `workerMode` to `interactive` before dispatching a smoke if needed, and note that Main should revert it after closeout. In that case:
